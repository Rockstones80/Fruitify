...
const AdminNavbar = ({setOpen, isOpen}) => {
  return (
    ...
    <div className="flex justify-between py-3 px-5 bg-white">
      <div>
      <p className=" font-extrabold text-xl text-center mt-8 py-6"><span className=" text-[#4880FF]">Dash</span>Stack</p>
      </div>
      <div className="flex gap-6">
        <div className="flex gap-3 my-auto">
            <div onClick={()=>setOpen(!isOpen)} className="my-auto cursor-pointer"><IoMdMenu size={20} /></div>
            <div className="text-gray-300 relative">
                <input type="text" className="rounded-full w-[388px] px-6 h-9 text-sm bg-gray-100" placeholder="    Search..." />
                <div className="absolute top-3 px-4 z-20"><IoMdSearch /></div>
            </div>
        </div>
      </div>
